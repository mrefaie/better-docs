...
module.exports = function bundle(Components, out, config) {
  ...
  if (vueComponents.length) {
    init =
      ...
      import VueWrapper from '${path.relative(
        absoluteOut,
        path.join(__dirname, VUE_WRAPPER)
      )}';\n
  ...
  if (reactComponents.length) {
    init =
      ...
      import ReactWrapper from '${path.relative(
        absoluteOut,
        path.join(__dirname, REACT_WRAPPER)
      )}';\n
      window.React = React;\n
  ...
  const cmd = `NODE_ENV=production parcel build ${entry} --out-dir ${outDist}`;
